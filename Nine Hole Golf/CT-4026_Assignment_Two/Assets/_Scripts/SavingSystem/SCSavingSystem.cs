...
public class SCSavingSystem : MonoBehaviour {
	public static void SaveData(StoreScript sc) {
		...
		string path = Application.persistentDataPath + "/store.bin";

		// This is creating and opening a file stream
		FileStream stream = new FileStream(path, FileMode.Create);

		// this is all of the data to save
		StoreClass data = new StoreClass(sc);
	...
	public static StoreClass LoadData() {
		...
		if (File.Exists(path)) {
			...
			StoreClass data = formatter.Deserialize(stream) as StoreClass;
			stream.Close();
			return data;
		} else {
