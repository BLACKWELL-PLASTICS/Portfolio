...
public class PlayerTwoGrenadeScript : MonoBehaviour {
	public string left;
	public string right;
	...
	void Update() {
		...
		if (Input.GetButton("Fire2_JoyOne") && GameObject.Find("Grenade" + this.gameObject.name.ToString()).GetComponent<Rigidbody2D>() == null) {
			float lXPos = this.gameObject.transform.position.x;
			float lYPos = this.gameObject.transform.position.y;
			if (rightLeft) {
		...
		if (Input.GetButtonUp("Fire2_JoyOne")) {
			float lXPos = this.gameObject.transform.position.x;
			float lYPos = this.gameObject.transform.position.y;
			if (rightLeft) {
