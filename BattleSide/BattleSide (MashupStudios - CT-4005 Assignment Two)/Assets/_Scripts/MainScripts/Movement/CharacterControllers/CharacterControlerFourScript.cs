...
public class CharacterControlerFourScript : MonoBehaviour {
	...
	void Update() {
		// if the player 4 is moving left
		if (Input.GetAxis("Horizontal_JoyThree") == -1) {
		...
		// if the player 4 is moving right
		if (Input.GetAxis("Horizontal_JoyThree") == 1) {
			Vector2 movement = new Vector2(5.0f, 0);
		...
		// if the player 4 presses A
		if (Input.GetButtonDown("Fire1_JoyThree")) {
			if (isGrounded == true) {
