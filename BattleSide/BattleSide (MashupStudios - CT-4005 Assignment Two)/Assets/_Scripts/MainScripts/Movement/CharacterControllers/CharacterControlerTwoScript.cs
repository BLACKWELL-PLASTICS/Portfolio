...
public class CharacterControlerTwoScript : MonoBehaviour {
	...
	void Update() {
		// if the player 2 is moving left
		// else, the player isnt moving left
		...
		// if the player 2 is moving right
		// else, the player isnt moving right
		if (Input.GetAxis("Horizontal_JoyOne") == 1) {
			Vector2 movement = new Vector2(5.0f, 0);
		...
		// if the player 2 presses A
		if (Input.GetButtonDown("Fire1_JoyOne")) {
			// if is the player grounded
			if (isGrounded == true) {
